...
async def test_aggregates_per_congress_with_loyalty(
    ...
    assert (
        rec["yea"] + rec["nay"] + rec["present"] + rec["notVoting"] + rec["other"]
    ) == rec["totalVotes"]
    assert (
        abs(rec["participationRate"] - 190 / 200) < 1e-9
    )  # (total - notVoting) / total
...
_HOUSE_DATA = {
    "s": {
        "nodes": [
            ...
            {
                "congress": 118,
                "category": "election-of-the-speaker",
                "position": "Jeffries",
                "positions": 1,
            },
            {
...
async def test_buckets_by_meaning_house_impeachment_and_speaker(
    ...
    mock_graphql.post(govql_endpoint).mock(
        return_value=graphql_response(data=_HOUSE_DATA)
    )
    ...
    assert (
        r118["yea"] + r118["nay"] + r118["present"] + r118["notVoting"] + r118["other"]
    ) == 55
