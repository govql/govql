...
export default function Nominations(): ReactNode {
  ...
  // Distinct congress numbers, newest first
  const congresses = Array.from(
    new Set(sessionData.map((s) => s.congress))
  ).sort((a, b) => b - a);

  // Years available for the selected congress, newest first
  ...
  // Reset year when congress changes
  useEffect(() => {
    setSelectedYear('');
  }, [selectedCongress]);
  ...
  return (
    <Layout
      title="Senate Nomination Votes"
      description="Explore how each state's senators voted on presidential nominations"
    >
      <div className="container margin-vert--lg">
        <Heading as="h1">Senate Nomination Votes</Heading>
        <p>
          Select a congress and a state to see how that state's senators voted
          on each confirmation vote. Optionally filter by year within the
          congress. Data is queried live from the{' '}
          <a
            href="https://api.govql.us/graphql"
            target="_blank"
            rel="noreferrer"
          >
            GovQL GraphQL API
